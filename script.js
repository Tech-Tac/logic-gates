...
class Workspace extends Circuit {
  ...
  initializeCanvas() {
    ...
    const updateGrid = () => {
      this.ctx.canvas.style.backgroundSize = `${this.gridSize * (this.zoom / this.scale)}px ${this.gridSize * (this.zoom / this.scale)
        }px`;
      ...
      this.ctx.canvas.style.backgroundPosition = `${this.ctx.canvas.width / this.scale / 2 + (this.panX + this.gridSize / 2) * (this.zoom / this.scale)
        }px
            ...
            [
              {
                ...
                backgroundPosition: `${this.ctx.canvas.width / this.scale / 2 + (this.panX + this.gridSize / 2) * (zoom / this.scale)
                  }px
                                                  ${this.ctx.canvas.height / this.scale / 2 +
    ...
    document.addEventListener("keydown", (e) => {
      console.log(
        `received ${e.ctrlKey ? "ctrl+" : ""}${e.shiftKey ? "shift+" : ""}${e.alt ? "alt+" : ""}${e.metaKey ? "meta+" : ""
        }${e.key}`
      );
